...
function NavBar() {
  ...
  return (
    <header className='sticky top-0 z-52 backdrop-blur-lg bg-white/70 dark:bg-slate-900/70 border-b border-slate-200 dark:border-slate-800'>
      <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
        ...
        <nav className='hidden md:flex items-center gap-8'>
          {navLinks.map((link) => (
            <Link to={link.href} key={link.name} className='text-slate-700 dark:text-slate-300 hover:text-blue-600 dark:hover:text-blue-300 transition-colors duration-300'>
              {link.name}
            </Link>
          ))}
        </nav>
